...
const showRecipeByUserIdModel = async (id) => {
  console.log("model - showRecipeById");
  return new Promise((resolve, reject) =>
    Pool.query(
      `
        SELECT 
            recipe.*, category.name AS category, users.full_name AS author
        FROM recipe
            JOIN users ON recipe.user_id = users.id
            JOIN category ON recipe.category_id = category.id 
...
const inputRecipeModel = async (data) => {
  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      `INSERT INTO
            recipe (id, title, ingredient, photo, created_at, category_id, user_id)
        VALUES (
            '${id}', 
            '${title}', 
            '${ingredient}', 
