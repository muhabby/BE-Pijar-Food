...
const searchUsersDetailModel = async (data) => {
  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      `SELECT * FROM users WHERE ${searchBy} ILIKE '%${search}%' ORDER BY ${sortBy} ${sort} LIMIT ${limit} OFFSET ${page}`,
      (err, res) => {
        if (!err) {
          return resolve(res);
        } else {
...
const inputUsersModel = async (data) => {
  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      `INSERT INTO users (id, full_name, email, password, profile_picture, bio, created_at) VALUES (
            '${id}', '${full_name}', '${email}', '${password}', '${profile_picture}', '${bio}', NOW());`,
      (err, res) => {
        if (!err) {
          return resolve(res);
        } else {
...
const updateUsersModel = async (data) => {
  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      `UPDATE users SET updated_at=NOW(), id='${id}', full_name='${full_name}', email='${email}', password='${password}', profile_picture='${profile_picture}', bio='${bio}'
        WHERE id='${id}';`,
      (err, res) => {
        if (!err) {
          return resolve(res);
