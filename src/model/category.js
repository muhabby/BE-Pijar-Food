...
const searchCategoryDetailModel = async (data) => {
  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      `SELECT * FROM category WHERE ${searchBy} ILIKE '%${search}%' ORDER BY ${sortBy} ${sort} LIMIT ${limit} OFFSET ${page}`,
      (err, res) => {
        if (!err) {
          return resolve(res);
        } else {
...
const inputCategoryModel = async (data) => {
  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      `INSERT INTO category (name, description, picture, created_at, updated_at) VALUES (
            '${name}', '${description}', '${picture}', NOW(), NOW());`,
      (err, res) => {
        if (!err) {
          return resolve(res);
        } else {
...
const updateCategoryModel = async (data) => {
  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      `UPDATE category SET updated_at=NOW(), name='${name}', description='${description}', picture='${picture}' WHERE id='${id}';`,
      (err, res) => {
        if (!err) {
          return resolve(res);
        } else {
